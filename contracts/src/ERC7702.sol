...
contract ERC7702 {
    ...
    event Executed(address indexed caller, address indexed recipient, uint256 amount, bytes data);
    ...
    function execute(address _recipient, uint256 _amount, bytes memory _data) external returns (bool) {
        // Verify with MasterControl
        bool allowed = IMasterControl(masterControl).verify(_recipient, _amount);
